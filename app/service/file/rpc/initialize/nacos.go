...
func InitNacos(Port int) (registry.Registry, *registry.Info) {
	v := viper.New()
	v.SetConfigFile(consts.FileConfigFile)
	if err := v.ReadInConfig(); err != nil {
		msg := fmt.Sprintf("viper read file nacos config failed err:%v", err)
		log.Zlogger.Fatal(msg)
	}
	if err := v.Unmarshal(&config.GlobalNacosConfig); err != nil {
		msg := fmt.Sprintf("viper unmarshal file nacos config failed err:%v", err)
		log.Zlogger.Fatal(msg)
	}
