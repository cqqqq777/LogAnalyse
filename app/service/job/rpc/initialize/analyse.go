...
func InitAnalysis() analyseservice.Client {
	// init resolver
	...
	r := nacos.NewNacosResolver(nacosCli, nacos.WithGroup(consts.AnalyseGroup))
	if err != nil {
	...
	// create a new client
	c, err := analyseservice.NewClient(
		config.GlobalServerConfig.AnalysisSrvInfo.Name,
		client.WithResolver(r),                                     // service discovery
		client.WithLoadBalancer(loadbalance.NewWeightedBalancer()), // load balance
		client.WithMuxConnection(1),                                // multiplexing
		client.WithSuite(tracing.NewClientSuite()),
		client.WithClientBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: config.GlobalServerConfig.AnalysisSrvInfo.Name}),
	)
	if err != nil {
